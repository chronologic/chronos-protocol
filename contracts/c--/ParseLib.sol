...
library ParseLib {
        ...
        public returns (Transaction)
    {
        ...
        assembly {
            to := mload(add(_serializedTransaction, 32))
            callGas := mload(add(_serializedTransaction, 64))
            gasPrice := mload(add(_serializedTransaction, 96))
            value := mload(add(_serializedTransaction, 128))
            temporalUnit := mload(add(_serializedTransaction, 160))
        ...
        bytes memory data = getBytesAtLocation(_serializedTransaction, 352);
